...
char *color[] = {(char *) "red", (char *) "orange", (char *) "green", (char *) "cyan", (char *) "brown", (char *) "purple", (char *) "black", (char *) "violet", (char *) "peachbuf", (char *) "magenta"} ;
 	  ...
 	  if ((BooleanConstraints.length() == 0) || EvaluateConditions((char *) BooleanConstraints.c_str(), SitesCount))
	      ...
	      if (FoundValue)
		{
		  if (Param.FindPrecedence ((char *)PrecedencePattern.c_str()))
		  {
		    ...
		    sprintf (WebBuffer, "%s%s%s_%s?name=%s:%ld..%ld&span=%ld", WebAddress, EnhancerAddress, Param.GetSessionName(), Param.GetChName(), ChromosomeName, start_gene, end_gene, end_gene-start_gene) ;
...

{
  ...
  if (!seq.length()) 
    ...
    sprintf (buffer, "%s", "<TABLE BORDER=\"1\" CELLPADDING=\"2\" CELLSPACING=\"0\" WIDTH=\"200\" BGCOLOR=\"#FF0000\"><caption> <b> Cluster Sequence </b></caption><tr><td><TABLE BORDER=\"1\" CELLPADDING=\"2\" CELLSPACING=\"0\" WIDTH=\"100%\" BGCOLOR=\"#FFFFFF\"><tr><td><font color=black>%s</font>") ;
   htm[4] +=  buffer ;
