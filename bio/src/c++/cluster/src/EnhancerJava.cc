...
void GetNearByGenes (long Start, long End, string &GeneDetails, vector <Annotation *> &Alist, EnhancerParam &Param, string &done, string &xmlstr, string & geneinfo, bool &Found) 
{
	  ...
	  if (Add)
	    {
	      ...
	      char *NotesInfo = (char *) (*beginpos)->GetNotes((char *) "GeneName") ;
	      xmlstr += "<name>\n" ;
	      if (NotesInfo)
		{
	      ...
	      else
		{
	      ...
	      sprintf (buffer, "%s<td>%ld</td>", GeneStart.c_str(), (*beginpos)->GetStartPosition()) ;
	      GeneStart =  buffer ;
	      sprintf (buffer, "%ld\n", (*beginpos)->GetStartPosition()) ;
	      xmlstr +=  buffer ;
	      xmlstr += "</start>\n<end>\n" ;

	      sprintf (buffer, "%s<td>%ld</td>", GeneEnd.c_str(), (*beginpos)->GetEndPosition()) ;
	      GeneEnd =  buffer ;
	      ...
	      else
		{
	      ...
	      xmlstr += "</strand>\n" ;
	      NotesInfo = (char *) (*beginpos)->GetNotes((char *) "Blast") ;
	      if (NotesInfo)
		{
	      ...
	      else
		{
	      ...
	      NotesInfo = (char *) (*beginpos)->GetNotes((char *) "MolecularFunction") ;
	      if (NotesInfo)
		{
	...
	while (beginpos != Alist.end())
	  {
	    ...
	    xmlstr += "<gene>" ;
	    description.erase (description.begin(), description.end()) ;
	    NotesInfo = (char *) (*beginpos)->GetNotes((char *) "GeneName") ;
	    xmlstr += "<name>\n" ;
	    if (NotesInfo)
	      {
		if (strstr (GeneNames, NotesInfo))
  ...
  if (whichway == 'r' || whichway == 'b') 
	...
	
	{
	  ...
	  sprintf (buffer, "<td>%ld</td>", (*beginpos)->GetStartPosition()) ;
	  GeneStart += buffer ;
	      sprintf (buffer, "%ld\n", (*beginpos)->GetStartPosition()) ;
	      xmlstr +=  buffer ;
	      xmlstr += "</start>\n<end>\n" ;
	  sprintf (buffer, "<td>%ld</td>", (*beginpos)->GetEndPosition()) ;
	  GeneEnd += buffer ;
	      sprintf (buffer, "%ld\n", (*beginpos)->GetEndPosition()) ;
	      xmlstr +=  buffer ;
	      xmlstr += "</end\n<strand>\n" ;
