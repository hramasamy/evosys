 ...
 * queue. Each input queue has a priority, which determines the approximate
 * frequency at which its items are dequeued, and then added to the output
 * queue. E.g. if queue A has priority 9.5, and queue B has priority 0.5, then
 * on average, for every 100 items added to the output queue, 95 should come from
 * queue A, and 5 should come from queue B.
 * <p>
...
public abstract class Combiner<T>
{
     ...
     *
     * @param queue the input queue to add to this Combiner
     ...
     *        longer than isEmptyTimeout, then it is removed from the Combiner
     ...
     *
     * @param queue the input queue to remove from this Combiner
     */
    public abstract void removeInputQueue(BlockingQueue<T> queue)
    ...
    /**
     * Thrown to indicate a Combiner specific exception.
     */
